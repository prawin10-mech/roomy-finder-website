...
const CityCarousel = () => {
  ...
  useEffect(() => {
    const updateCarouselCities = () => {
      const slicedCities = cities.slice(
        currentCityIndex,
        currentCityIndex + (isMediumScreen ? 3 : 3)
      );
      const remainingCities = cities.slice(
        0,
        (isMediumScreen ? 3 : 3) - slicedCities.length
      );
  ...
  const renderCityImage = (city, imageSrc, altText, isActive, index) => {
    ...
    const position =
      index === 0 || index === 2 
        ? "relative"
        : "static";
    const overlap = index === 0 || index === 2 ? "0px" : "auto";
  ...
  const carouselItems = carouselCities.map((city, index) => {
    ...
    let styles = isActive
      ? {
          minHeight: { xs: 0, md: "20vh", lg: "30vh" },
          borderRadius: "10px",
          display: "flex",
  ...
  return (
    ...
    >
      ...
      <Box sx={{ display: "flex", width: "60%", maxWidth: "60%" }}>
        {carouselItems}
      </Box>
      <Box
        sx={{
